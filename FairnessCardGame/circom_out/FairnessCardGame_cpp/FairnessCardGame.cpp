...
void Range_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Range_2_run(uint ctx_index,Circom_CalcWit* ctx);
void Range_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Range_3_run(uint ctx_index,Circom_CalcWit* ctx);
...
void Main_6_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Main_6_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 9161;}

uint get_number_of_components() {return 36;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 6853;}
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],&circuitConstants[1]); // line circom 15
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template " << myTemplateName << " line 15. " <<  "Followed trace: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
{
...
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[1]].signalStart + 0],&circuitConstants[1]); // line circom 20
if (!Fr_isTrue(&expaux[0])) std::cout << "Failed assert in template " << myTemplateName << " line 20. " <<  "Followed trace: " << ctx->getTrace(myId) << std::endl;
assert(Fr_isTrue(&expaux[0]));
}
...
Fr_lt(&expaux[0],&lvar[221],&circuitConstants[7]); // line circom 275
while(Fr_isTrue(&expaux[0])){
...
Fr_lt(&expaux[0],&lvar[3],&circuitConstants[10]); // line circom 16
while(Fr_isTrue(&expaux[0])){
{
...
uint csoffset = mySignalStart+8860;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "rp1";
mySubcomponents[aux_create+i] = aux_cmp_num;
Range_2_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 75 ;
aux_cmp_num += 5;
}
}
...
uint csoffset = mySignalStart+9010;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "rp_suit1";
mySubcomponents[aux_create+i] = aux_cmp_num;
Range_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 75 ;
aux_cmp_num += 5;
}
}
...
uint csoffset = mySignalStart+9085;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "rp_suit2";
mySubcomponents[aux_create+i] = aux_cmp_num;
Range_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 75 ;
aux_cmp_num += 5;
...
uint csoffset = mySignalStart+3550;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mimc1_2";
mySubcomponents[aux_create+i] = aux_cmp_num;
MiMCSponge_5_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1770 ;
aux_cmp_num += 3;
...
uint csoffset = mySignalStart+7090;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mimc_value";
mySubcomponents[aux_create+i] = aux_cmp_num;
MiMCSponge_5_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1770 ;
aux_cmp_num += 3;
...
uint csoffset = mySignalStart+5320;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "mimc_suit";
mySubcomponents[aux_create+i] = aux_cmp_num;
MiMCSponge_5_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1770 ;
aux_cmp_num += 3;
...
// need to run sub component
assert(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
Range_2_run(mySubcomponents[cmp_index_ref],ctx);
}
{
uint cmp_index_ref = 1;
{
...
// need to run sub component
assert(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter));
Range_3_run(mySubcomponents[cmp_index_ref],ctx);
}
{
...
Fr_eq(&expaux[0],&signalValues[mySignalStart + 8],&signalValues[mySignalStart + 9]); // line circom 50
