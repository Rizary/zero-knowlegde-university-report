...
void MiMCHash_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void MiMCHash_3_run(uint ctx_index,Circom_CalcWit* ctx);
void MerkleTree_4_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void MerkleTree_4_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 19524;}

uint get_number_of_components() {return 45;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 14529;}
...
ctx->componentMemory[coffset].templateName = "MiMCHash";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
uint csoffset = mySignalStart+3;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "hashFunc";
mySubcomponents[aux_create+i] = aux_cmp_num;
...
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
ctx->componentMemory[coffset].subcomponents = new uint[23];
}
...
uint csoffset = mySignalStart+12435;
uint aux_dimensions[1] = {8};
for (uint i = 0; i < 8; i++) {
std::string new_cmp_name = "singleHash"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
...
uint aux_positions [7]= {0,1,2,3,4,5,6};
for (uint i_aux = 0; i_aux < 7; i_aux++) {
uint i = aux_positions[i_aux];
std::string new_cmp_name = "doubleHash"+ctx->generate_position_array(aux_dimensions, 1, i);
mySubcomponents[aux_create+i] = aux_cmp_num;
MiMCHash_3_create(csoffset,aux_cmp_num,ctx,new_cmp_name,myId);
csoffset += 1773 ;
aux_cmp_num += 4;
}
}
if (!Fr_isTrue(&circuitConstants[3])) std::cout << "Failed assert in template " << myTemplateName << " line 27. " <<  "Followed trace: " << ctx->getTrace(myId) << std::endl;
...
Fr_lt(&expaux[0],&lvar[5],&circuitConstants[17]); // line circom 39
while(Fr_isTrue(&expaux[0])){
{
...
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[5])) + 9)];
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[5])) + 0)]].signalStart + 0]);
}
{
PFrElement aux_dest = &lvar[5];
// load src
Fr_add(&expaux[0],&lvar[5],&circuitConstants[3]); // line circom 39
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[5],&circuitConstants[17]); // line circom 39
...
Fr_lt(&expaux[0],&lvar[5],&circuitConstants[15]); // line circom 47
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = ((1 * Fr_toInt(&lvar[5])) + 8);
{
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
// end load src
...
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
MiMCHash_3_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[4])) + 9)]);
}
// run sub component if needed
if(!(--ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter)){
MiMCHash_3_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
// end load src
Fr_copy(aux_dest,&ctx->signalValues[ctx->componentMemory[mySubcomponents[((1 * Fr_toInt(&lvar[5])) + 8)]].signalStart + 0]);
}
{
PFrElement aux_dest = &lvar[3];
// load src
Fr_add(&expaux[0],&lvar[3],&circuitConstants[5]); // line circom 52
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[4];
// load src
Fr_add(&expaux[0],&lvar[4],&circuitConstants[5]); // line circom 53
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[2];
// load src
Fr_add(&expaux[0],&lvar[2],&circuitConstants[3]); // line circom 54
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &lvar[5];
// load src
Fr_add(&expaux[0],&lvar[5],&circuitConstants[3]); // line circom 47
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
Fr_lt(&expaux[0],&lvar[5],&circuitConstants[15]); // line circom 47
...
MerkleTree_4_create(1,0,ctx,"main",0);
MerkleTree_4_run(0,ctx);
}
